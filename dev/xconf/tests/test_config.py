...
def test_difference_only_diffs():
	default = {"a": 1, "nested": {"x": 1, "y": 2}, "keep": 0}
	override = {"a": 5, "nested": {"x": 9}}
	diff = xconf.difference(default, override)
	xtest.equal(diff, {"a": 5, "nested": {"x": 9}})

def test_difference_no_change():
	default = {"a": 1, "b": 2}
	diff = xconf.difference(default, default)
	xtest.same(diff, xconf._UNSET)

def test_missing_nested():
	default = {"top": {"a": 1, "b": 2}}
	override = {"top": {"a": 1}}
	miss = xconf.missing(default, override)
...
def test_strict_omissions_raises():
	with tempfile.TemporaryDirectory() as tmp:
		...
		with open(os.path.join(tmp, "z.json"), "w") as f:
			json.dump({"a": 9}, f)
		xconf.configure(tmp)
		xconf.no_omissions("z", strict=True)
		def do_read():
			xconf.read("z")
...
def test_save_diff_only():
	with tempfile.TemporaryDirectory() as tmp:
		...
		with open(os.path.join(tmp, "default", "w.json"), "w") as f:
			json.dump({"a": 1, "b": 2}, f)
		xconf.configure(tmp)
		xconf.save("w", {"a": 7, "b": 2}, diff_only=True)
