...
is a defect. Classify honestly: if a function can't be labeled A/B/C/D, that
is a signal to refactor, not a loophole. If all copies of some logic can be
described by one spec plus context parameters, it is one system — the copies
are a bookkeeping defect.

## R6 — Meet the catalog before writing
